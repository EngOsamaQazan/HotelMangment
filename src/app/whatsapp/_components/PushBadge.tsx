...
export function PushBadge({ push }: { push: WhatsAppPushState }) {
  ...
  if (push.isSubscribed) {
    return (
      <button
        ...
        className="tap-44 inline-flex items-center justify-center gap-1.5 text-[11px] px-2.5 py-1.5 rounded-full border border-green-200 bg-green-50 text-green-700 hover:bg-green-100"
        aria-label="الإشعارات مفعّلة — اضغط للإيقاف"
  ...
  return (
    <button
      ...
      className={cn(
        "tap-44 inline-flex items-center justify-center gap-1.5 text-[11px] px-2.5 py-1.5 rounded-full border",
        denied
          ? "border-red-200 bg-red-50 text-red-600"
          : "border-gray-200 bg-white text-gray-600 hover:bg-gray-50",
      )}
      aria-label={denied ? "الإشعارات محظورة من المتصفح" : "تفعيل الإشعارات المكتبية"}
